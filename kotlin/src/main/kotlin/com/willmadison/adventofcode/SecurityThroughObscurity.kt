...
data class Room(val sectorId: Int = 0, val checksum: String = "") {
    fun isReal(givenChecksum: String) = givenChecksum == checksum
}

fun String.parseRoom(): Pair<Room, String> {
    val (sectorId, givenChecksum) = this.substringAfterLast("-").split("[", "]")
    val encryptedRoomName = this.substringBeforeLast("-")

    val checksum = encryptedRoomName.replace("-", "").groupBy { it }
            .mapValues { it.value.count() }
            .toList().sortedWith(compareByDescending<Pair<Char, Int>> { it.second }.thenBy { it.first }).take(5).joinToString("") { it.first.toString() }

    return Room(sectorId.toInt(), checksum) to givenChecksum
}

fun main(args: Array<String>) {
    val candidates = """aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
...
yhwooebeaz-nwilwcejc-ydkykhwpa-owhao-160[skuyi]""".split("\n")

    var sum = 0

    candidates.forEach { candidate ->
        val (room, givenChecksum) = candidate.parseRoom()
        if (room.isReal(givenChecksum)) {
            sum = sum.plus(room.sectorId)
        }
    }

    println("The sum of the sector ids for all the real rooms is $sum.")
